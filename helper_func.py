...
async def get_short(url, client):
    ...
    try:
        ...
        client.LOGGER(__name__).warning(f"[Shortener Error] {e}")
...
async def shortner_panel(client, query_or_message):
    ...
    if shortner_enabled:
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f"https://{short_url}/api?api={short_api}&url=https://google.com&alias=test", timeout=5) as test_response:
                    status = "✓ ᴡᴏʀᴋɪɴɢ" if test_response.status == 200 else "✗ ɴᴏᴛ ᴡᴏʀᴋɪɴɢ"
        except:
            status = "✗ ɴᴏᴛ ᴡᴏʀᴋɪɴɢ"
    else:
        status = "✗ ᴅɪsᴀʙʟᴇᴅ"
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(api_url, timeout=10) as response:
                ...
                if rjson.get("status") == "success" and response.status == 200:
